...
@UIApplicationMain
class AppDelegate: UIResponder, UIApplicationDelegate, OnboardingManagerDelegate, UNUserNotificationCenterDelegate {
    var window: UIWindow?

    private let logger = Logger.shared(named: "AppDelegateLogger")
    var workpermit: Workpermit<OnlineODataProvider>!
    ...
    func onboarded(onboardingContext: OnboardingContext, onboarding _: Bool) {
        var configurationURL = onboardingContext.sapcpmsSettingsParameters?.url(forDestination: "workpermit")
        if configurationURL == nil {
            configurationURL = onboardingContext.authenticationURL
        }
        if configurationURL == nil {
            configurationURL = URL(string: "https://hcpms-s0019641412trial.hanatrial.ondemand.com/workpermit")!
        }
    ...
    private func setRootViewController() {
        DispatchQueue.main.async {
            let splitViewController = UIStoryboard(name: "PermitType", bundle: Bundle.main).instantiateViewController(withIdentifier: "MainViewController") as! UINavigationController     
//splitViewController.delegate = self
    ...
    private func configureOData(_ urlSession: SAPURLSession, _ serviceRoot: URL) {
        let odataProvider = OnlineODataProvider(serviceName: "Workpermit", serviceRoot: serviceRoot, sapURLSession: urlSession)
        // Disables version validation of the backend OData service
        // TODO: Should only be used in demo and test applications
        odataProvider.serviceOptions.checkVersion = false
        self.workpermit = Workpermit(provider: odataProvider)
        // To update entity force to use X-HTTP-Method header
        self.workpermit.provider.networkOptions.tunneledMethods.append("MERGE")
    }
}
