//
// PermitTypeMasterViewController.swift
// IOSKT
//
...
class PermitTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var workpermit: Workpermit<OnlineODataProvider> {
        return self.appDelegate.workpermit
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([PermitType]?, Error?) -> Void) -> Void)?
    private var entities: [PermitType] = [PermitType]()
    private let logger = Logger.shared(named: "PermitTypeMasterViewControllerLogger")
    private let okTitle = NSLocalizedString("keyOkButtonTitle",
                                            value: "OK",
                                            comment: "XBUT: Title of OK button.")
    public func fetchWorkPermit(_ completionHandler: @escaping ([PermitType]?, Error?) -> Void) {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let permittype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(permittype.orderID!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.workpermit.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
         self.fetchWorkPermit() { entities, error in
            if let error = error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! PermitTypeDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].orderID ?? ""
...
extension PermitTypeMasterViewController: EntitySetUpdaterDelegate {
