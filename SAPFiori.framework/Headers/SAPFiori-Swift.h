...
typedef SWIFT_ENUM(NSInteger, FUIBarcodeScanMode, closed) {
...
///
/// \endcodeThe <code>FUIButton</code> also implements the <code>FUIBackgroundSchemeSupporting</code> protocol, which enables developers to inform the control whether it is being presented against a ‘light’ background, or ‘dark’ background.  The default style of the button will adapt to the background color scheme; defaults to <code>.lightBackground</code>.
/// <h2>Size Calculation</h2>
/// The <code>FUIButton</code> calculates an <code>intrinsicContentSize</code> from its <code>imageView</code> and <code>titleLabel</code>, its various insets, and its <code>layoutMargins</code>.
...
/// :nodoc:
@property (nonatomic, readonly, copy) NSString * _Nonnull defaultNUIClass;
/// :nodoc:
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
/// :nodoc:
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
/// Initializer with <code>type == .custom</code> and <code>style == .none</code>
- (nonnull instancetype)init;
/// :nodoc:
...
/// Developer should use the <code>FUIKPIViewItem</code> API set to build KPI content, and set to the <code>kpiItems: [FUIKPIViewItem]</code> property.
/// The optional <code>trendSemanticColor: UIColor?</code> will be applied to the <code>trend</code> and <code>trendImage</code> properties, if not <code>nil</code>.
/// Developer should configure the <code>chartView: FUIChartView</code>.
/// <blockquote>
/// Note:  the default Card configuration for the <code>FUIChartView</code> will cause only the first and last category title to be displayed.
///
/// </blockquote>
/// <h2>Data Source</h2>
/// Developer should implement the <code>FUIChartViewDataSource</code>, to supply data to the card’s <code>chartView</code> property.
/// <h2>Usage</h2>
/// \code
...
@interface FUIImageCollectionItemView : FUIBaseDrawingView
/// :nodoc:
- (CGSize)systemLayoutSizeFittingSize:(CGSize)targetSize withHorizontalFittingPriority:(UILayoutPriority)horizontalFittingPriority verticalFittingPriority:(UILayoutPriority)verticalFittingPriority SWIFT_WARN_UNUSED_RESULT;
/// :nodoc:
@property (nonatomic, readonly) CGSize intrinsicContentSize;
...
/// container.searchResultsViewController.tableView.dataSource = searchResultsDataSource
/// container.searchResultsViewController.tableView.delegate = searchResultsDelegate
...
/// container.searchResultsViewController.searchBar.delegate = searchResultsDelegate
///
/// container.contentViewController.headlineText = "VA Palo Alto Health Care Sys wraps to two lines..."
/// container.contentViewController.didSelectTitleHandler = {
///    print("Developer Select Handler Called!")
/// }
/// container.contentViewController.subheadlineText = "Medical Center"
/// container.contentViewController.tableView.dataSource = contentDataSource
/// container.contentViewController.tableView.delegate = contentDelegate
...
///            self.container.contentViewController.tableView.dataSource = newDataSource
...
///
/// \endcode
SWIFT_CLASS("_TtC8SAPFiori50FUIHorizontalScrollDynamicSizeCollectionViewLayout")
...
/// <code>FUIKPIHeader</code> extends <code>UIView</code> for a tableview header displaying KPI items, of type FUIKPIView and FUIKPIProgressView. A maximum of four KPI values can be displayed in the header, hence if more than four items are provided in the items array as input to the header, only the first four are diplayed and the rest of the items are ignored. When both FUIKPIView and FUIKPIProgressView items are provided, the items are sorted such that the FUIKPIProgressViews are displayed first, followed by the FUIKPIViews. Only FUIKPIProgessViews with chartSize of ‘large’can be displayed in the header , if an FUIProgressView of charSize  ‘small’is supplied, it is ignored. Based on the number of KPI items that can be shown based on the screen size, a page control is used to display multiple pages in the header, as required. The kpi items in the header can be enabled or disabled. The default KPI header background is of color scheme .darkBackground, hence to display the kpi items with the appropriate tintColor as per the fiori design guidelines for iOS, it is required that the kpi items supplied to the header have a color scheme of .darkBackground as well. The developer can choose to supply a custom tint color for the kpis. It is important to set the colorScheme of the kpi before the call to override the tintColor, as shown below.( since setting the colorScheme resets the tintColor to the defaults, if the calls are not in order, then the custom tintColors are not displayed.)
/// \code
///
/// kpiView.colorScheme = .darkBackground
...
/// kpiView1.colorScheme = .darkBackground
///
...
/// kpiChartLarge.colorScheme = .darkBackground
///
...
@interface FUIKPIHeader : FUIBaseDrawingView
/// :nodoc:
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
/// :nodoc:
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
...
/// </ul>
/// Color setting:
/// Setting text color of <code>valueTextView</code> for a state using setTintColor(_:for:) api. Currently <code>.disabled</code> and <code>.normal</code> are supported.
/// \code
/// cell.setTintColor(UIColor.red, for: .normal)
///
/// \endcoderemark:
/// The <code>font-color</code> attribute will be overridden by <code>tint-color-disabled</code> attribute when cell is switched to <code>disabled</code> state. DO NOT set <code>textColor</code> for <code>valueTextView</code> when cell is in <code>disabled</code> state!
/// The following is an example of usage in an application <code>UITableViewController</code>:
/// important:
/// The app’s <code>UITableViewController</code> must subclass <code>FUIFormTableViewController</code>
/// \code
...
@property (nonatomic, readonly, weak) IBOutlet FUITextViewExt * _Null_unspecified noteText SWIFT_UNAVAILABLE_MSG("'noteText' has been renamed to 'valueTextView'");
/// :nodoc:
- (nonnull instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString * _Nullable)reuseIdentifier OBJC_DESIGNATED_INITIALIZER;
/// :nodoc:
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
...
/// <img src="../ReadmeImages/FUIKeyValueFormCell.png?raw=true" alt="FUIKeyValueFormCell" width="376">
/// The developer should set the following properties on the cell, in their implementation of <code>UITableViewDataSource</code> <code>cellForRow(at:)</code> function:
/// <ul>
///   <li>
///     <code>keyName</code>: The key name of the property.
...
/// fdlFUIListPickerTVC_cancelItem
/// fdlFUIListPickerTVC_sectionHeaderTitleLabel
/// fdlFUIListPickerTVC_listTextLabel
///
/// \endcode
...
- (nonnull instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString * _Nullable)reuseIdentifier OBJC_DESIGNATED_INITIALIZER SWIFT_AVAILABILITY(ios,introduced=3.0) SWIFT_UNAVAILABLE;
...
/// A View Controller component used in the <code>FUIDetailPanelContainer</code>.  It is up to the developer to set the tableView dataSource and delegate.  Typically not used directly by developer.
/// <h2>Usage:</h2>
/// \code
...
/// A View Controller component within the <code>FUIDetailPanelContainer</code> as the <code>contentViewController</code>.  This controller shows additional details of a business object.  It is up to the developer to set the tableView dataSource and delegate.  Typically not used directly by developer.
/// <h2>Available:</h2>
/// <ul>
///   <li>
...
/// contentViewController.headlineText = "Headline Text"
/// contentViewController.subheadlineText = "Subheadline Text"
/// contentViewController.didSelectTitleHandler = {
///    print("didSelectTitleHandler called!")
/// }
/// contentViewController.tableView.delegate = delegate
/// contentViewController.tableView.dataSource = dataSource
/// contentViewController.tableView.register(FUIMapDetailBaseObjectTableViewCell.self, forCellReuseIdentifier: FUIMapDetailBaseObjectTableViewCell.reuseIdentifier)
/// contentViewController.tableView.estimatedRowHeight = 100
/// contentViewController.tableView.rowHeight = UITableViewAutomaticDimension
///
/// \endcode
...
/// <h2>Usage:</h2>
/// \code
/// let searchResultsViewController = FUIMapDetailPanelSearchResultsViewController()
/// searchResultsViewController.isApplyingBlurBackground = true
/// searchResultsViewController.tableView.dataSource = tableViewDataSource
/// searchResultsViewController.tableView.delegate = tableViewDelegate
/// searchResultsViewController.tableView.register(FUIMapDetailTagObjectTableViewCell.self, forCellReuseIdentifier: FUIMapDetailTagObjectTableViewCell.reuseIdentifier)
/// searchResultsViewController.searchBar.delegate = searchBarDelegate
///
/// \endcode
...
@interface FUIMapDetailStatusView : FUIBaseDrawingView
/// :nodoc:
- (nonnull instancetype)init;
/// :nodoc:
- (CGSize)systemLayoutSizeFittingSize:(CGSize)targetSize withHorizontalFittingPriority:(UILayoutPriority)horizontalFittingPriority verticalFittingPriority:(UILayoutPriority)verticalFittingPriority SWIFT_WARN_UNUSED_RESULT;
...
/// Chart & KPI content View for presentation in <code>FUIObjectHeader</code>.  Set an instance of this view to the <code>FUIObjectHeader.detailContentView</code> property, for correct layout.
/// <h2>Usage</h2>
/// \code
...
///
/// \endcodeSupported <code>ObjectHeaderView</code> attributes:
/// \code
/// tint-color (Color)
...
/// text-line-clamp (Integer)
///
/// \endcodeSupported <code>ImageView</code> class paths:
/// \code
...
@interface FUIObjectHeaderView : FUIBaseDrawingView
/// :nodoc:
- (CGSize)systemLayoutSizeFittingSize:(CGSize)targetSize withHorizontalFittingPriority:(UILayoutPriority)horizontalFittingPriority verticalFittingPriority:(UILayoutPriority)verticalFittingPriority SWIFT_WARN_UNUSED_RESULT;
/// :nodoc:
@property (nonatomic, readonly) CGSize intrinsicContentSize;
...
///
/// \endcode<h2>Theming</h2>
/// Supported <code>ObjectView</code> class paths:
/// \code
/// fdlFUIObjectView {}
///
/// \endcodeSupported <code>ObjectView</code> attributes:
/// \code
/// tint-color (Color)
/// background-color (Color)
///
...
/// tint-color (Color)
///
/// \endcodeAdditional supported style classes
/// \code
...
/// text-line-clamp: Integer;
/// text-align: NSTextAlignment;
SWIFT_CLASS("_TtC8SAPFiori23FUIPlaceholderImageView")
...
@interface FUIPlaceholderView : FUIBaseDrawingView
/// :nodoc:
@property (nonatomic, readonly, copy) NSString * _Nonnull defaultNUIClass;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
...
/// \endcode<ul>
///   <li>
///     The table view which profile header is added to has <code>isScrollEnable</code> set to true.
///   </li>
///   <li>
///     The table view which profile header is added to is embeded in a navigation controller and navigation bar is not hidden.
///   </li>
/// </ul>
/// <h2>Views Available in FUIProfileHeader:</h2>
/// <ul>
///   <li>
///     <code>imageView</code>: a UIImageView. The image size is set to <code>80px</code> by <code>80px</code>.  Use <code>image</code> or <code>imageView.image</code>to set image.
///   </li>
///   <li>
...
///
///
/// \endcode<em>Important:</em> There is no way to add a view as a subview of UITableView in storyboard/nib file. We highly recommand setup profile header programmatically, but if you really need to use storyboard/nib file, follow the instructions below:
/// <ol>
///   <li>
...
/// </ul>
/// Color settings:
/// Setting tintColor for add button for a state using setTintColor(_:for:) api. Currently <code>disabled</code>, <code>normal</code>, <code>selected</code> are supported. <code>selected</code> means switch is turned on.
/// \code
/// cell.setTintColor(UIColor.red, for: .normal)
///
/// \endcodeThe following is an example of usage in an application <code>UITableViewController</code>:
...
///
/// \endcoderemark:
/// The <code>font-color</code> attribute will be overridden by <code>tint-color-disabled</code> attribute when cell is switched to <code>disabled</code> state. DO NOT set <code>textColor</code> for <code>valueTextView</code> when cell is in <code>disabled</code> state!  Use the <code>setTintColor(UIColor(), for: .disabled)</code> to set the color.
/// The following is an example of usage in an application <code>UITableViewController</code>:
/// important:
/// The app’s <code>UITableViewController</code> must subclass <code>FUIFormTableViewController</code>
/// \code
...
///
/// \endcode
SWIFT_CLASS("_TtC8SAPFiori20FUITextFieldFormCell")
@interface FUITextFieldFormCell : FUIInlineValidationTableViewCell
/// The value of the property.
@property (nonatomic, copy) NSString * _Nonnull value;
/// The key name of the property.
@property (nonatomic, copy) NSString * _Nullable keyName;
...
/// :nodoc:
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
/// The color of the text displayed by the text view.
...
/// The background view of this <code>FUITimelineCell</code>.
@property (nonatomic, readonly, weak) IBOutlet UIView * _Null_unspecified cardBackground;
/// :nodoc:
- (nonnull instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString * _Nullable)reuseIdentifier OBJC_DESIGNATED_INITIALIZER;
/// :nodoc:
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
...
/// Use <code>FUITimelineNode</code> static properties, to return standard Fiori Design Language node images.
@property (nonatomic, readonly, strong) IBOutlet FUIImageView * _Null_unspecified nodeImageView;
/// The background view of this <code>FUITimelineMarkerCell</code>.
@property (nonatomic, readonly, weak) IBOutlet UIView * _Null_unspecified cardBackground;
/// The background view of of the area on the left of timeline.
@property (nonatomic, readonly, weak) IBOutlet UIView * _Null_unspecified timelineBackground;
/// :nodoc:
...
@interface FUIUserConsentPageView : FUIBaseDrawingView
/// :nodoc:
...
/// to more details. There are 2 buttons in the toolbar, “Agree” and “Disagree”, if the consent form is
...
/// the <code>title</code> poperty,<code>body</code> containing the details, an <code>actionTitle</code> ,which when clicked invokes the
...
/// and actionTitle properties. Eg. When both <code>title</code>and <code>titleAttributedText</code> are supplied, only
...
/// \endcode<h2>Theming</h2>
/// Supported class paths:
/// fdlFUIConsentPageView_title {}
/// fdlFUIConsentPageView_body {}
/// fdlFUIConsentPageView_actionTitle {}
/// Supported properties:
/// font-color: Color;
/// font-style: UIFontTextStyle;
/// text-line-clamp: Integer;
