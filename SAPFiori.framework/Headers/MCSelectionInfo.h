...
typedef struct
{
    ...
    /**
     Indicates whether the selection is associated with the first (0) or second (1) touch location.
     Any selection that's indirectly associated to a touch location will be given the touch location index it corresponds with.
     For example in Pulse where we sometimes automatically select the previous point in addition to the user's selection.
     */
    int touchIndex;
    
    /**
     Events that indicate the specific update for this selection.
     */
    int events;
